...
/**
 * This class helps reads information about Biography books from user as well as from file and prints it
...
public class Biography extends NonFiction{
	...
	protected void readBookInfoFromUser(Scanner scan) {
		...
		while(loopingTest==0) {
			...
			try {
					...
					System.out.print("Enter biography title: ");	//asks user to enter biography title
					name=scan.next();
				...
				System.err.println("Input Mismatch Exception while reading biography properties");
			}
		}
	...
	/**
	 * Takes in input from the file for various properties of the Biography class and its subclass
